  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-sm flex-col justify-center px-4 py-10">
      <div className={card}>
        ...
        <p className="mb-4 text-xs text-muted">Not a client login. Access is restricted to one account.</p>
        <form action={submit}>
          ...
          <button className={`${primaryButton} w-full`} disabled={busy} type="submit">
            {busy ? "..." : mode === "login" ? "Log in" : "Register founder account"}
          </button>
        </form>
        {error && (
        ...
        <button
          ...
          type="button"
        >
          {mode === "login" ? "First time -- register the founder account" : "Already registered -- log in"}
        </button>
      </div>
    </main>
  );
