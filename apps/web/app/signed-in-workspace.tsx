...
import { api, billing, integrations, type ActivityLogEntry, type BillingStatus, type Claim, type DashboardSummary, type Evidence, type EvidenceQualitySummary, type ExtractedEvidence, ACTION_STATUSES, type ActionStatus, type Incident, type Integrations, type Postmortem, type PreviousDraft, type SimilarIncident, type StatusPageUpdate, type UpiPricing, webhooks, type WirePricing } from "./api";
...
import { accountUpdateSchema, evidenceSchema, firstError, incidentSchema, integrationsSchema, paymentReferenceSchema, statusPageUpdateSchema, wireCurrencySchema } from "./validation";
...
// The signed-in workspace: everything a customer sees after the front door.
// Loaded on demand by workspace.tsx so the signed-out landing (the page a
// prospect from anywhere lands on) never downloads it. The founder
// dashboard is one level further down, loaded only for the founder.
const FounderDashboard = dynamic(() => import("./founder-dashboard"), {
  ssr: false,
  loading: () => <DashboardSkeleton />,
});

export default function SignedInWorkspace({ user, setUser }: { user: AuthUser; setUser: (user: AuthUser | null) => void }) {
...
function SubscribeGate({
  ...
  // UPI and international wire are the only rails -- the card processor
  // was removed on 2026-09-13 on the owner's instruction.
...
function ManageBilling() {
  ...
  // Every subscription here was paid by UPI or wire and approved by hand;
  // there is no self-service portal because there is no card processor.
  // Renewing is another payment through the same form above.
  return (
    <Card className={card}>
      ...
      <p className="mt-1 text-xs text-muted">
        {status.subscription_status === "expired"
          ? "Your access has expired -- make a new UPI or wire payment above to reactivate, or "
          : "Paid via UPI or wire -- to renew, make another payment above, or "}
